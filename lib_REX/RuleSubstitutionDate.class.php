...
class RuleSubstitutionDate extends Rule
{
	...
	public function launch($pdo)
	{
		...
		while($dt = $q->fetch())
		{
			...
			$result[$dt[(string)$this->targetColumn]] = new DateTime($year.'-'.$month.'-'.$day. ' '.$hour.':'.$min.':'.$sec);
			$result[$dt[(string)$this->targetColumn]] = $result[$dt[(string)$this->targetColumn]]->format($this->date_mask);
		}
		$q->closeCursor();
